...
class TSP():
    ...
    def update2(self,frame):
        ...
        x = [self.X[i] for i in newTour]
        x.append(x[0])
        y = [self.Y[i] for i in newTour]
        y.append(y[0])
        self.line.set_data(x, y)
        self.iterTimes.append(frame)
        self.iterDis.append(newDis)
    ...
    def on_start(self):
        ...
        if self.ui.radioButton_2.isChecked():#LS
            ...
            self.animation = FuncAnimation(self.canvas1.fig, self.update2,interval = 0,blit=True,frames = self.frameNum * 20,repeat=False)
