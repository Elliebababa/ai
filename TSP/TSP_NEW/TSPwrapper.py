...
class TSP():
    ...
    def update2(self,frame):
        ...
        ymin, ymax = min(self.iterDis),max(self.iterDis)
        self.canvas2.ax.set_ylim(ymin*0.99, ymax)
    ...
    def on_start(self):
        ...
        if self.ui.radioButton_2.isChecked():#LS
            ...
            self.animation = FuncAnimation(self.canvas1.fig, self.update2,interval = 0,blit=False,frames = self.frameNum * 20,repeat=False)
            self.animation = FuncAnimation(self.canvas2.fig, self.update2,init_func=self.init,interval = 0,blit=True,frames = self.frameNum * 20,repeat=False)
    ...
    def sa(self):
        #内循环
        if not(len(self.iterTimes) %(self.frameNum//100) == 0):
