...
class TSP():
    def __init__(self,X,Y):
        ...
        self.canvas3 = TSPCanvas()
        self.ui.verticalLayout_6.addWidget(self.canvas3)
        self.line3, = self.canvas3.ax.plot(self.iterTimes,self.iterDis,'b-')
        self.plotBest()
    ...
    def plotBest(self):
        file = open("gr202.opt.tour", "r")
        lines = file.readlines()
        x = []
        y = []
        for line in lines[5:len(lines)-3]:
            params = int(line)
            x.append(self.X[params-1])
            y.append(self.Y[params-1])
        file.close()
        x.append(x[0])
        y.append(y[0])
        self.line3, = self.canvas3.ax.plot(x,y,'b-')
    ...
    def resetUp(self):
        ...
        self.iterTimes,self.iterDis=[],[]
        self.line2, = self.canvas2.ax.plot(self.iterTimes,self.iterDis,'b-',animated =True,label="search")
        self.canvas2.ax.set_ylim(38000,60000)
        self.canvas2.ax.plot([0,10000],[40160,40160],'r-',animated =False,label="optimal")
        self.canvas2.ax.legend(bbox_to_anchor=(1.0, 1), loc=1, borderaxespad=0.)
        self.canvas2.ax.text(0, 40500,'optimal distance : 40160')
    ...
    def update2(self,frame):
        #内循环
        if not(len(self.iterTimes) % 200 == 0):
            newTour,newDis = self.combine(self.curTour)
            #print(r.random() , math.exp((self.curDis-newDis)/self.T))
        ...
        #外循环
            ...
            str = "退火温度:%.2f"%(self.T)+"      当前最短距离:%d"%self.curDis
        ...
        self.canvas2.ax.set_ylim(38000, max(self.iterDis))
    ...
    def init(self):
        ...
        self.canvas2.ax.set_ylim(38000,60000)
        self.canvas2.ax.set_yticks([38000,60000])
        return self.line2,
    ...
    def on_start(self):
        ...
        if self.ui.radioButton_2.isChecked():#LS
            if self.ui.radioButton_6.isChecked():#SWAP2
                ...
                self.optMode = 4
            self.animation = FuncAnimation(self.canvas1.fig, self.update,interval = 0,blit=True,frames = self.frameNum,repeat=False)
            self.animation = FuncAnimation(self.canvas2.fig, self.update,init_func=self.init,interval = 0,blit=True,frames = self.frameNum,repeat=False)
        elif self.ui.radioButton.isChecked():#SA
            self.dropRate = float(self.ui.rate1.text())
            self.T = float(self.ui.T1.text())
            self.animation = FuncAnimation(self.canvas1.fig, self.update2,interval = 0,blit=True,frames = self.frameNum,repeat=False)
            self.animation = FuncAnimation(self.canvas2.fig, self.update2,init_func=self.init,interval = 0,blit=True,frames = self.frameNum,repeat=False)
    ...
    def ls(self):
        ...
        if newDis < self.dis:
            self.dis = newDis
            self.tour = newTour.copy()
            str = "次数:%d"%(self.iterTimes[-1]+1)+"      当前最短距离:%d"%self.dis
            self.ui.listWidget.addItem(str)
            self.ui.listWidget.setCurrentRow(self.ui.listWidget.count()-1)
