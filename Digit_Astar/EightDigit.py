...
class SolveEight():
 ...
 #       self.initState = [8,2,4,6,1,3,7,5,0]
    ...
    def run(self,callback):
        ...
        for i in range(9):
            for j in range(i):
                if not(self.initState[i] == 0 or self.initState[j] == 0)  and self.initState[j] > self.initState[i]:
                    p0+=1
                if not(self.finalState[i] == 0 or self.finalState[j] == 0) and self.finalState[j] > self.finalState[i]:
                    pt+=1
        if not (p0 % 2 == pt % 2):
            print("无解！")
            return
        ...
        while (len(self.openList)):
            ...
            if top.value == 0:#结束
                ...
                callback("","","",path)
                return
            zeroPos = top.stateList.index(0)
            
            for nextPos in move[zeroPos]:
